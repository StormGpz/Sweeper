...
window["i18n"] = {
    ...
    init(language) {
        ...
        initPolyglot(data);
        this.inst = polyInst;
        logDebug("=====i18n,polyInst=====",polyInst);
    },
    t(key, opt) {
        if (polyInst) {
            let str = polyInst.t(key, opt);
...
module.exports = {
    ...
    updateSceneRenderers() { // very costly iterations
        ...
        for (let i = 0; i < allLocalizedLabels.length; ++i) {
            ...
            if (!label.node.activeInHierarchy) continue;
            label.updateLabel();
        }
        ...
        for (let i = 0; i < allLocalizedSprites.length; ++i) {
            ...
            if (!sprite.node.activeInHierarchy) continue;
            sprite.updateSprite(window.i18n.curLang);
        }
        cc.gg.evt.emit(cc.GGEventType.I18N_UPDATE);
    }
};